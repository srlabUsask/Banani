...
class ImgPipeline:
    ...
    def analysis(self, img_object, markers, fname,params):
        ...
        leafDescriptorPath = "/mnt/data/result_hdfs_seg_analysis/" + only_name[len(only_name)-2]+".p"

        pickle.dump(img_object, sftp.open(leafDescriptorPath, 'wb'))
        sftp.close()
        # End code for local file system
    ...
    def commonTransform(self, params):
        def common_transform(datapack):
            ...
            # only_name = img_name.split('.')
            #
            # leafDescriptorPath = "/mnt/data/result_hdfs_seg_transform/" + only_name[len(only_name)-2]+".p"
            # pickle.dump(procsd_obj, sftp.open(leafDescriptorPath, 'wb'))
            # sftp.close()
            # End code for local file system
    ...
    def commonAnalysisTransform(self, params):
        def common_transform(datapack):
            ...
            processedimg, stats = self.analysis(img, model, fname, params)
...
class ImgMatching(ImgPipeline):
    ...
    def commonEstimate(self, params):
        def common_estimate(datapack):
            ...
            fname, img, procsd_obj = datapack
            try:
                kp, descriptors = super.estimate_feature(procsd_obj,params)
            except Exception as e:
                descriptors = None
                print(e)
...
def loadExecuteHDFS(sc, hdfdir, pipes):
    ...
    rdd = rdd.filter(lambda x: (x[1] is not None) and (hasattr(x[1], 'shape')))
    return rdd
...
def common_analysis(pipes, rdd, params):
    ...
    #print("Checking analysing:" + str(rdd.count()))
    return rdd
...
def match_model(pipes, rdd, params):
    ...
    key_pnts, feature2 = pipes.estimate_feature(pipes.convert(img_to_match),featurename )
