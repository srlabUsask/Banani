...
class heat_inventory:
    ...
    ansible_ssh_private_key_file = "~/.ssh/hdpspark.pem"
    nodes_name= "hdpspark-master"

    # templates
    host_entry = Template('$ipaddress             ansible_connection=ssh  ansible_ssh_user=$ssh_user api_hostname  ansible_ssh_private_key_file=$private_key_file')
...
$nodes
    ...
    def load_heat_output(self):
        #self.heat_output = json.dumps(subprocess.Popen("heat output-show sparkhadoop-stack --all 2>&1 | grep -v WARN | grep -v 'You can upgrade' | grep -v  'SNI' | grep -v '^192' | grep -v '^206' | grep -v 'Insecure' ", shell=True, stdout=subprocess.PIPE).stdout.read())
        foo = subprocess.Popen("echo [ && heat output-show hdpspark-stack --all --format json  | sed -e 's/}/},/g' | head -n -1 && echo } && echo ]", shell=True, stdout=subprocess.PIPE).stdout.read()
        print(foo)
